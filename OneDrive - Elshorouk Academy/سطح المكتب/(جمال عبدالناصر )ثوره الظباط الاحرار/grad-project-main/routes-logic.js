document.addEventListener('DOMContentLoaded', () => {
    ...
    function getRouteColor(routeId, routeName) {
        ...
        if (name.includes('shorouk') || name.includes('شروق') || String(routeId) === '9') return '#ef4444';
        ...
        Object.keys(zones).forEach((zoneName, zIndex) => {
            ...
            zoneRoutes.forEach(rt => {
                ...
                const stationsHtml = stationsArr.length > 0 
                    ? stationsArr.map(s => `<span style="background:${color}15; border:1px solid ${color}30; padding:6px 14px; border-radius:12px; font-size:0.8rem; font-weight:900; color:${color}; display:inline-flex; align-items:center; gap:6px;"><i class="fas fa-map-pin" style="color:${color}; opacity:0.8;"></i> ${s}</span>`).join('')
                ...
                routeCard.innerHTML = `
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:15px; padding-left:10px;">
                        ...
                        <div style="font-size:1.6rem; font-weight:900; color:#ef4444;">${mockProgress}%</div>
                    </div>
                    ...
                    <div style="width:100%; height:4px; background:var(--border-color); border-radius:4px; margin-bottom:20px; overflow:hidden; margin-left:10px; width:calc(100% - 10px);">
                        <div style="width:${mockProgress}%; height:100%; background:#ef4444; border-radius:4px;"></div>
                    </div>
